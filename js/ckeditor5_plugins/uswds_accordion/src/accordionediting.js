...
export default class AccordionEditing extends Plugin {
  ...
  _defineSchema() {
    ...
    schema.addChildCheck((context, childDefinition) => {
      ...
      if (
        (context.endsWith('accordionContent') || context.endsWith('accordionTitle')) &&
  ...
  _defineConverters() {
    ...
    const options = this.editor.config.get( 'uswds_accordion.options' );
    let $defaultBordered = false;
    let $accordionClass = 'usa-accordion';

    if ( options ) {
      $defaultBordered = !!options[0]['bordered'];
    ...
    conversion.for('dataDowncast').add(dispatcher => {
      dispatcher.on('insert:accordionTitle', (evt, data, conversionApi) => {
        ...
        const h4 = writer.createContainerElement('h4', { class: 'usa-accordion__heading' });

        const buttonDiv = writer.createEditableElement('div', { class: 'usa-accordion__button', type: 'button', 'aria-expanded': false });
